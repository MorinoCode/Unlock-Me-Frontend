...
const ProfilePage = () => {
  ...
  useEffect(() => {
    if (formData.countryCode && availableLocations.length > 0) {
      const selectedLocation = availableLocations.find(
        (l) => l.countryCode === formData.countryCode
      );
  ...
  useEffect(() => {
    const fetchFullProfile = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/user/user/${currentUser._id}`, {
          credentials: "include",
        });
        if (res.ok) {
          const data = await res.json();
          ...
          setFormData({
            ...
            birthday:
              data.birthday && data.birthday.year
                ? data.birthday
                : { day: "", month: "", year: "" },
  ...
  const fetchAllInterestOptions = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/user/onboarding/interests-options`,
        { credentials: "include" }
      );
      if (res.ok) {
        const data = await res.json();
    ...
    if (
      ...
      questionsData[0].questions.forEach((q) => {
        const found = existingAnswers.find(
          (ans) => ans.questionText === q.questionText
        );
        if (found) {
    ...
    <div className="profile-page">
          ...
          <div className="profile-page__cropper-content">
            ...
            <div className="profile-page__cropper-controls">
              <input
                ...
                className="profile-page__cropper-slider"
              />
      ...
      <div className="profile-page__layout">
        <aside className="profile-page__sidebar">
          ...
          <span
            className={`profile-page__badge profile-page__badge--${formData.subscription.plan.toLowerCase()}`}
          >
          ...
          <div className="profile-page__nav">
            <button
              className={`profile-page__nav-btn ${
                activeTab === "general" ? "profile-page__nav-btn--active" : ""
              }`}
            ...
            <button
              className={`profile-page__nav-btn ${
                activeTab === "gallery" ? "profile-page__nav-btn--active" : ""
              }`}
            ...
            <button
              className={`profile-page__nav-btn ${
                activeTab === "security" ? "profile-page__nav-btn--active" : ""
              }`}
              ...
              <div className="profile-page__form">
                ...
                <div className="profile-page__form-group profile-page__form-group--full">
                  ...
                  <div className="profile-page__form-row">
                    <input
                      className="profile-page__input profile-page__input--small"
                      placeholder="DD"
                      name="day"
                ...
                {formData.gallery.map((img, i) => (
                  <div key={i} className="profile-page__gallery-item">
                    <img
                      src={img}
                      alt=""
                      className="profile-page__gallery-img"
                    />
                    <button
                      className="profile-page__gallery-remove"
                      onClick={() =>
                        setFormData({
                          ...formData,
            ...
            <div className="profile-page__section">
              ...
              <div className="profile-page__interests-list">
                  ...
                  <div key={cat} className="profile-page__interest-container">
                    <div className="profile-page__interest-row">
                      <div className="profile-page__interest-info">
                        ...
                        <span className="profile-page__interest-status">
                          {formData.categories[cat].length} Answers
                        </span>
                      </div>
                      <button
                            ...
                            <div className="profile-page__quiz-options">
                              {q.options.map((opt) => (
                                <button
                                  ...
                                  className={`profile-page__quiz-opt-btn ${
                                    newAnswers[q._id]?.selectedText === opt.text
                                      ? "profile-page__quiz-opt-btn--active"
                                      : ""
                                  }`}
                                >
                                  {opt.text}
                ...
                <div className="profile-page__add-interest-wrapper">
                  {!isAddingInterest ? (
                    <button
                      className="profile-page__btn profile-page__btn--full"
                      onClick={fetchAllInterestOptions}
                    ...
                    <div className="profile-page__interest-form">
                      <div className="profile-page__form-group">
                        ...
                        >
                          ...
                          {allInterestOptions
                            .filter((c) => !formData.categories[c.label])
                            .map((c) => (
                      ...
                      {selectedNewCat && (
                        <button
                          className="profile-page__btn profile-page__btn--primary profile-page__btn--full"
                          onClick={() => submitInterest(selectedNewCat)}
                          disabled={isSaving}
                        >
