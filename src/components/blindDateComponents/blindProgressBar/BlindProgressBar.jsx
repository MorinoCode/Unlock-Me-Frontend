...
const BlindProgressBar = ({ currentStage, currentIndex }) => {
  ...
  const label = isStage3 ? t("blindDate.messages") : t("blindDate.question");
  ...
  return (
    <div className="blind-progress-bar">
      <div className="blind-progress-bar__info">
        <span className="blind-progress-bar__stage">
          {t("blindDate.stage", { num: currentStage })}
        </span>
      ...
      <div className="blind-progress-bar__track">
        ...
        {Array.from({ length: maxSteps }).map((_, step) => {
          ...
          return (
            <div
              key={step}
              className={`blind-progress-bar__step ${
                isActive ? "blind-progress-bar__step--active" : ""
              }`}
            >
